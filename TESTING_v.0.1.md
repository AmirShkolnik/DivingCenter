...
| Epic | Title | User Story | Expected Result | Status |
|------|-------|------------|-----------------|--------|
| EPIC 1: User Authentication and Profile Management | Create User Account | As a user, I want to create an account so that I can access personalized features of the diving center website. | User can successfully create an account with valid information. Account is stored in the database and user receives a confirmation message via toastify. | ✔️ |
| EPIC 1: User Authentication and Profile Management | Secure Login | As a user, I want to log in to my account securely so that I can access my personal information and bookings. | User can log in with correct credentials. Failed login attempts are logged. Successful login grants access to personalized features. | ✔️ |
| EPIC 1: User Authentication and Profile Management | Edit Profile Information | As a user, I want to edit my profile information so that I can keep my details up to date. | User can access and modify their profile information. Changes are saved correctly in the database and reflected immediately in the user interface. | ✔️ |
| EPIC 1: User Authentication and Profile Management | Password Reset | As a user, I want to reset my password if I forget it so that I can regain access to my account. | User can request a password reset. A unique reset link is sent to their email. The link allows for password change, which is then updated in the database. | ✔️ |
| EPIC 1: User Authentication and Profile Management | Manage User Accounts | As an admin, I want to manage user accounts so that I can assist users with account-related issues. | Admin can view, edit, and delete user accounts. Changes made by admin are immediately reflected in the system. | ✔️ |
| EPIC 1: User Authentication and Profile Management | View User Profiles | As an admin, I want to view user profiles so that I can understand our user base better. | Admin can access a list of all user profiles and view detailed information for each user. | ✔️ |
| EPIC 2: Course Catalog and Booking System | Browse Courses | As a user, I want to browse available diving courses so that I can find courses that interest me. | User can browse a list of available courses with basic information displayed. | ✔️ |
| EPIC 2: Course Catalog and Booking System | View Course Details | As a user, I want to view detailed information about each course so that I can make informed decisions. | User can view detailed information about each course, including description, schedule, and prerequisites. | ✔️ |
| EPIC 2: Course Catalog and Booking System | Book Course Online | As a user, I want to book a course online so that I can secure my spot conveniently. | User can book a course online, receive a booking confirmation, and see the booking reflected in their account. | ✔️ |
| EPIC 2: Course Catalog and Booking System | View Booked Courses | As a user, I want to view my booked courses so that I can keep track of my upcoming diving activities. | User can view a list of their booked courses with details such as date, time, and location. | ✔️ |
| EPIC 2: Course Catalog and Booking System | Add New Courses | As an admin, I want to add new courses to the catalog so that I can keep our offerings up to date. | Admin can add new courses to the catalog, which are then visible to users. | ✔️ |
| EPIC 2: Course Catalog and Booking System | Manage Course Bookings | As an admin, I want to manage course bookings so that I can ensure proper allocation of resources. | Admin can view, edit, and manage all course bookings, ensuring proper resource allocation. | ✔️ |
| EPIC 3: Community Engagement and Social Features | Post Course Reviews | As a user, I want to post reviews for courses I've taken so that I can share my experiences with others. | User can post reviews for courses, which are then visible to other users. | ✔️ |
| EPIC 3: Community Engagement and Social Features | Read Course Reviews | As a user, I want to read reviews from other divers so that I can make informed decisions about courses. | User can read reviews posted by other users for each course. | ✔️ |
| EPIC 3: Community Engagement and Social Features | Participate in Forums | As a user, I want to participate in discussion forums so that I can engage with other diving enthusiasts. | User can participate in forums, post new topics, and reply to existing discussions. | ✔️ |
| EPIC 3: Community Engagement and Social Features | Share Diving Experiences | As a user, I want to share my diving experiences through a personal feed so that I can connect with other divers. | User can share diving experiences through a personal feed, visible to other users. | ✔️ |
| EPIC 3: Community Engagement and Social Features | Moderate User Content | As an admin, I want to moderate user-generated content so that I can maintain a positive community environment. | Admin can review, approve, or delete user-generated content to maintain community standards. | ✔️ |
| EPIC 3: Community Engagement and Social Features | Manage Community Events | As an admin, I want to create and manage community events so that I can foster user engagement. | Admin can create and manage community events, which are visible to users. | ✔️ |
| EPIC 4: Content Management and Administration | Update Course Information | As an admin, I want to update course information easily so that I can keep the course catalog current. | Admin can update course information, which is then reflected in the course catalog. | ✔️ |
| EPIC 4: Content Management and Administration | Manage Static Content | As an admin, I want to manage static page content so that I can keep information accurate and up-to-date. | Admin can manage static content on the website, ensuring it is accurate and up-to-date. | ✔️ |
...
| EPIC 7: Advanced Platform Features and Integrations | Access Personalized Diving Recommendations | As a user, I want to receive personalized recommendations for courses, equipment, and dive sites based on my preferences and experience level so that I can enhance my diving journey. | User receives personalized recommendations based on their preferences and experience. | ✔️ |
